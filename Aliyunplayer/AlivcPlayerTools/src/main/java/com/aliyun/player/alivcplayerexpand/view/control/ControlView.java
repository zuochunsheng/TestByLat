...
public class ControlView extends RelativeLayout implements ViewAction, ITheme {
    ...
    private void setViewListener() {
        ...
        mScreenShot.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mOnScreenShotClickListener != null && (!PlayParameter.IS_TRAILER
        ...
        mScreenRecorder.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mOnScreenRecoderClickListener != null && (!PlayParameter.IS_TRAILER
                        && mVideoPosition < AliyunVodPlayerView.TRAILER)) {
...
//        }
    ...
    private void judgeCurrentPlayingVideo() {
        if (mAdvPosition == null || (!PlayParameter.IS_VIDEO)) {
            return;
        }
        switch (mAdvPosition) {
            case ONLY_START:
...
//    }
    ...
    private void updateSmallInfoBar() {
        if (mAliyunScreenMode == AliyunScreenMode.Full) {
            ...
            //先设置小屏的info数据
            if (PlayParameter.IS_VIDEO && !mInScreenCosting) {
                setAdvVideoTotalDuration();
                mSmallMutiSeekbar.calculateWidth();
                if (isSeekbarTouching) {
                    //用户拖动的时候，不去更新进度值，防止跳动。
                } else {
//                    mSmallMutiSeekbar.setSecondaryProgress(mVideoBufferPosition);
...
//                    mLargeMutiSeekbar.setSecondaryProgress(mVideoBufferPosition);
...
//            mScreenShot.setVisibility(GONE);
    ...
    public void setOnDanmuClickListener(OnDanmuClickListener listener) {
        this.mOnDanmuClickListener = listener;
    }
    ...
    public void hideNativeLargeSeekBar(final long advTime, final long sourceTime, final MutiSeekBarView.AdvPosition advPosition) {
        this.mAdvDuration = advTime;
        this.mSourceDuration = sourceTime;
        this.mAdvPosition = advPosition;
        if (mLargeMutiSeekbar != null) {
            ...
            mLargeMutiSeekbar.post(new Runnable() {
                @Override
                public void run() {
                    mLargeMutiSeekbar.setTime(advTime, sourceTime, advPosition);
                }
            });
        }
        if (mLargeSeekbar != null) {
